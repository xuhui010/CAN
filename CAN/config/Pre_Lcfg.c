...
void Pre_StateData_Write(Pre_StateType state, unsigned char data) //将报文相关数据存入预充状态数据结构体中
{
	if (state = PRE_STATE_FAULT)
	{
		Pre_StateData.fault = data;      //存储故障数据
	}
	else if (state = PRE_STATE_TIME)
	{
		Pre_StateData.time = data;       //存储时间数据
	}
	else if (state = PRE_STATE_VOLTAGE)
	{
...
uint8 Pre_StateData_Fault(void)             //预充故障函数初始化
{
	return Pre_StateData.fault;             //返回故障参数值
}

uint8 Pre_StateData_Time(void)              //预充时间函数初始化
{
